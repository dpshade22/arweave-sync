...
export class VaultSyncModal extends Modal {
    ...
    private async initializeFiles() {
        ...
        this.remoteFiles = this.buildFileTree(this.plugin.settings.remoteUploadConfig);
        this.filesToImport = await this.getFilesToImport(this.plugin.settings.remoteUploadConfig);
    ...
    private async renderImportView() {
        ...
        this.localContainer = container.createDiv({ cls: "import-files" });
        ...
        this.renderFileTree(this.filesToImport, this.localContainer, false);

        if (this.filesToImport.length === 0) {
            this.localContainer.createEl("p", {
    ...
    private renderFileTree(
        ...
        const headerEl = containerEl.createEl("h3", {
            text: isSource ? (this.isExportView ? "Local Vault" : "Remote Files") : (this.isExportView ? "Files to Export" : "Files to Import"),
        });
    ...
    private async getFilesToImport(remoteUploadConfig: UploadConfig): Promise<FileNode[]> {
        const filesToImport: FileNode[] = [];

        for (const [filePath, remoteFileInfo] of Object.entries(remoteUploadConfig)) {
